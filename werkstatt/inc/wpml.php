...
function thb_language_switcher() {
	$host = 'werkstatt.fuelthemes.net';
	$thb_ls = ot_get_option('thb_ls', 'on');
	if ($thb_ls !== 'off') {
		if ( function_exists('icl_get_languages') || $_SERVER['HTTP_HOST'] === $host || function_exists('icl_get_languages')) {
						...
						if(1 < count($languages)){
							if (function_exists('pll_the_languages')) { // Polylang
								foreach($languages as $l){
									echo esc_attr($l['current_lang'] ? $l['id'] : '');
								}
							} else { // WPML
								foreach($languages as $l){
					...
					if(1 < count($languages)){
						foreach($languages as $l){
							if (function_exists('pll_the_languages')) {
								if (!$l['current_lang']) {
									echo '<li><a href="'.$l['url'].'" title="'.$l['name'].'">'.$l['name'].'</a></li>';
								}
							} else {
								if (!$l['active']) {
				...
				if(1 < count($languages)){
					foreach($languages as $l){
						if (function_exists('pll_the_languages')) {
							...
							echo '<a href="'.$l['url'].'"'.$class.' title="'.$l['name'].'">'.$l['id'].'</a>';
						} else {
							$class = $l['active'] ? ' class="active"' : '';  
							echo '<a href="'.$l['url'].'"'.$class.' title="'.$l['native_name'].'">'.$l['language_code'].'</a>';
						}
					}
				} else {
