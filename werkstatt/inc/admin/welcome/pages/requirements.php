<?php 
	...
	// WP Remote Get Check
	$posting['wp_remote_get']['name'] = esc_html__( 'Remote Get', 'woocommerce');
...
<table class="widefat" cellspacing="0">
	<thead>
		<thead>
			<tr>
				<td><?php esc_html_e( 'Requirements', 'werkstatt' ); ?></td>
				<td></td>
			</tr>
		</thead>
		<?php if ( function_exists( 'ini_get' ) ) : ?>
			<tr>
				<td data-export-label="Server Memory Limit">
					<?php esc_html_e( 'Server Memory Limit', 'werkstatt' ); ?>:
				</td>
				<td>
					<?php
					...
					<?php else: ?>
						<mark class="<?php echo esc_attr($mark); ?>">
								...
								<?php esc_html_e('The recommended value is 256M.', 'werkstatt'); ?>
						</mark>
					<?php endif; ?>
			...
			<tr>
				<td data-export-label="PHP Version"><?php esc_html_e( 'PHP Version:', 'werkstatt' ); ?></td>
				<td>
					<?php 
					...
					?>
						...
						<?php else: ?>
							<mark class="<?php echo esc_attr($mark); ?>">
									...
									<?php esc_html_e('Required PHP Version is at least 5.6. WordPress <a href="https://wordpress.org/about/requirements/" target="_blank">recommends using at least PHP 7.0.</a> ', 'werkstatt'); ?>
							</mark>
						<?php endif; ?>
					<?php	} ?>
				</td>
				...
				<td>
					...
					<?php else: ?>
						<mark class="<?php echo esc_attr($mark); ?>">
								...
								<?php esc_html_e('The recommended value is 30.', 'werkstatt'); ?>
						</mark>
					<?php endif; ?>
			...
			<td>
				...
				<?php else: ?>
					<mark class="<?php echo esc_attr($mark); ?>">
							...
							<?php esc_html_e('The recommended value is 12M.' , 'werkstatt'); ?>
					</mark>
				<?php endif; ?>
			</td>
		</tr>
				...
				<tr>
					...
					<td>
						<mark class="<?php echo esc_attr($mark); ?>">
							...
							<span><?php echo ! empty( $post['note'] ) ? esc_html__( $post['note'] ) : ''; ?></span>
						</mark>
					</td>
				</tr>
				<?php
