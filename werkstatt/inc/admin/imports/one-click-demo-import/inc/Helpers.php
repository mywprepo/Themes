...
class Helpers {
	...
	public static function import_file_info( $selected_import_files ) {
		...
		sprintf(
			__( 'Files info:%1$sSite URL = %2$s%1$sData file = %3$s%1$sWidget file = %4$s%1$sCustomizer file = %5$s%1$sRedux files:%1$s%6$s%1$sTheme Options file = %7$s%1$s', 'pt-ocdi' ),
			PHP_EOL,
			get_site_url(),
			...
			empty( $selected_import_files['theme_options'] ) ? esc_html__( 'not defined!', 'pt-ocdi' ) : $selected_import_files['theme_options']
		);
	}
