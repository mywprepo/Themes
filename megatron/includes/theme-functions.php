...
/**
 * Check Page Custom Style
 * If not exists then create page_id.css
 * *******************************************************
 */
if (!function_exists('g5plus_check_page_custom_style')) {
	function g5plus_check_page_custom_style() {
		if (is_singular() || is_404()) {
			$custom_page_dir = G5PLUS_THEME_DIR . 'assets/css/pages/';
			if (file_exists($custom_page_dir . get_the_ID() . '.css')) {
				return true;
			}

			/**
			 * Create page custom style
			 * *******************************************************
			 */
			$page_id = get_the_ID();
			...
			if (!file_exists($custom_page_dir)) {
				wp_mkdir_p($custom_page_dir);
			}
			$wp_filesystem->put_contents($custom_page_dir . get_the_ID() . '.css', $css, FS_CHMOD_FILE);

			if (!file_exists(G5PLUS_THEME_DIR . 'assets/css/pages/' . get_the_ID() . '.css')) {
				return false;
			}
			return true;
		}
...
/**
 * Remove custom page style after save post
 * *******************************************************
 */
if (!function_exists('g5plus_after_save_post_remove_custom_page')) {
	function g5plus_after_save_post_remove_custom_page($post_id, $post) {
		$custom_page_dir = G5PLUS_THEME_DIR . 'assets/css/pages/';
		if (file_exists($custom_page_dir . get_the_ID() . '.css')) {
			unlink ($custom_page_dir . get_the_ID() . '.css');
		}
	}
	add_action( 'save_post', 'g5plus_after_save_post_remove_custom_page', 10, 2 );
}

/**
 * Remove Custom page dir when after save options
 * *******************************************************
 */
if (!function_exists('g5plus_after_save_option_remove_page_dir')) {
	function g5plus_after_save_option_remove_page_dir() {
		require_once(ABSPATH . 'wp-admin/includes/file.php');
		WP_Filesystem();
		global $wp_filesystem;
		$custom_page_dir = G5PLUS_THEME_DIR . 'assets/css/pages/';
		if (file_exists($custom_page_dir)) {
			$wp_filesystem->rmdir($custom_page_dir, true);
		}
	}
	add_action('g5plus_after_save_option', 'g5plus_after_save_option_remove_page_dir');
}
