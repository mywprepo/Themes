...
class OZY_FooterText_Widget extends WP_Widget {
	...
	function OZY_FooterText_Widget() {
		...
		parent::__construct( 'footertext-widget', __('(Enjooy) Footer Text', 'vp_textdomain'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Copyright © Enjooy 2014', 'vp_textdomain'), 'language_switcher' => true );
		$instance = wp_parse_args( (array) $instance, $defaults ); ?>

		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Title:', 'vp_textdomain'); ?></label>
...
class OZY_SocialBar_Widget extends WP_Widget {
	...
	function OZY_SocialBar_Widget() {
		...
		parent::__construct( 'socialbar-widget', __('(Enjooy) Social Bar', 'vp_textdomain'), $widget_ops, $control_ops );
	}
...
class OZY_CustomMenu_Widget extends WP_Widget {
	...
	function OZY_CustomMenu_Widget() {
		...
		parent::__construct( 'custommenu-widget', __('(Enjooy) Custom Menu', 'vp_textdomain'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'active_menu' ); ?>"><?php _e('Menu:', 'vp_textdomain'); ?></label>
			<?php
				$menus = get_terms('nav_menu');
			?>
            <select id="<?php echo $this->get_field_id( 'active_menu' ); ?>" name="<?php echo $this->get_field_name( 'active_menu' ); ?>" style="width:100%;">
            	<?php
					foreach($menus as $menu){
					   	echo '<option value="'. $menu->slug .'" '. selected( $menu->slug, $instance['active_menu'], false ) .'>'. $menu->name .'</option>' . PHP_EOL;
					}
				?>
            </select>
		</p>
...
class OZY_LatestPosts_Widget extends WP_Widget {
	...
	function OZY_LatestPosts_Widget() {
		...
		parent::__construct( 'latestposts-widget', __('(Enjooy) Latest Posts', 'vp_textdomain'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'type' ); ?>"><?php _e('Type:', 'vp_textdomain'); ?></label>
			<?php
            ...
            <select id="<?php echo $this->get_field_id( 'type' ); ?>" name="<?php echo $this->get_field_name( 'type' ); ?>" style="width:100%;">
            	<?php
					foreach ( $type_arr as $type ) {
					   	echo '<option value="'. $type .'" '. selected( $type, $instance['type'], false ) .'>'. $type .'</option>' . PHP_EOL;
					}
				?>
            </select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_type' ); ?>"><?php _e('Post Type:', 'vp_textdomain'); ?></label>
			<?php
				$args = array(
				   'public'   => true,
				   '_builtin' => false
            ...
            <select id="<?php echo $this->get_field_id( 'post_type' ); ?>" name="<?php echo $this->get_field_name( 'post_type' ); ?>" style="width:100%;">
            	<?php
					foreach ( $post_types as $post_type ) {
					   	echo '<option value="'. $post_type .'" '. selected( $post_type, $instance['post_type'], false ) .'>'. $post_type .'</option>' . PHP_EOL;
					}
				?>
            </select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'order' ); ?>"><?php _e('Order:', 'vp_textdomain'); ?></label>
			<?php
            ...
            <select id="<?php echo $this->get_field_id( 'order' ); ?>" name="<?php echo $this->get_field_name( 'order' ); ?>" style="width:100%;">
            	<?php
					foreach ( $order_arr as $order ) {
					   	echo '<option value="'. $order .'" '. selected( $order, $instance['order'], false ) .'>'. $order .'</option>' . PHP_EOL;
					}
				?>
            </select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e('Order By:', 'vp_textdomain'); ?></label>
			<?php
            ...
            <select id="<?php echo $this->get_field_id( 'orderby' ); ?>" name="<?php echo $this->get_field_name( 'orderby' ); ?>" style="width:100%;">
            	<?php
					foreach ( $orderby_arr as $orderby ) {
					   	echo '<option value="'. $orderby .'" '. selected( $orderby, $instance['orderby'], false ) .'>'. $orderby .'</option>' . PHP_EOL;
					}
				?>
            </select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'post_per_page' ); ?>"><?php _e('Count:', 'vp_textdomain'); ?></label>
...
class OZY_Flickr_Widget extends WP_Widget {
	function OZY_Flickr_Widget() {
		parent::__construct('flickr_widget', '(Enjooy) Flickr Widget', array('classname' => 'flickr', 'description' => __('(Enjooy) Flickr Widget for user photo stream!', 'vp_textdomain')), array('id_base' => 'flickr_widget'));
		}
	...
	function widget($args, $instance)
	{
		...
		if($user_name && $number) {
			...
			if(isset($userid->stat) && $userid->stat == 'fail') {
				if(isset($userid->message)) {
					echo __('Error: ', 'vp_textdomain') . $userid->message;
				}
			}else{
				if($userid->user->id!='') {
					...
					echo '<p>'. __('Invalid flickr username.', 'vp_textdomain') .'</p>';
				}
			}
		}
		if($under_text!=''){
	...
	function form($instance)
	{
		...
		<p>
			...
			<input style="width:100%;" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" value="<?php echo $instance['title']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('user_name'); ?>">Username:</label>
			<input style="width:100%;" id="<?php echo $this->get_field_id('user_name'); ?>" name="<?php echo $this->get_field_name('user_name'); ?>" value="<?php echo $instance['user_name']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>">Number of photos to show:</label>
			<input style="width:100%;" id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" value="<?php echo $instance['number']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('under_text'); ?>">Type text under widget(Optional):</label>
			<textarea style="resize:none; width:100%; height:50px;" id="<?php echo $this->get_field_id('under_text'); ?>" name="<?php echo $this->get_field_name('under_text'); ?>"><?php echo $instance['under_text']; ?></textarea>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('img_widht'); ?>">Image width:</label>
			<input style="width:100%;" id="<?php echo $this->get_field_id('img_widht'); ?>" name="<?php echo $this->get_field_name('img_widht'); ?>" value="<?php echo $instance['img_widht']; ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('img_height'); ?>">Image Height:</label>
			<input style="width:100%;" id="<?php echo $this->get_field_id('img_height'); ?>" name="<?php echo $this->get_field_name('img_height'); ?>" value="<?php echo $instance['img_height']; ?>" />
		</p>		
	<?php
	}
	function update($new_instance, $old_instance)
