<?php
	class myHelper{
		...
		*/		
		...
		function get_custom_font($font_name) { //, $font_weight = 'normal', $font_style = 'normal'
			if($font_name) {
				...
				if(isset($font->ID)) {
					$font_grp = vp_metabox('ozy_enjooy_meta_font.ozy_enjooy_meta_font_group', null, $font->ID);
					if(is_array($font_grp) && count($font_grp) > 0) {						
						foreach($font_grp as $fnt) {
							$id = $fnt['ozy_enjooy_meta_font_id'];
							if(!in_array($id, $this->active_custom_fonts)) {
								$fnt_arr = array();
								$fnt_arr['title'] 	= $font->post_title;
								$fnt_arr['id'] 		= $id;
								$fnt_arr['eot'] 	= $fnt['ozy_enjooy_meta_font_eot'];
								$fnt_arr['woff'] 	= $fnt['ozy_enjooy_meta_font_woff'];
								$fnt_arr['ttf'] 	= $fnt['ozy_enjooy_meta_font_ttf'];
								$fnt_arr['svg'] 	= $fnt['ozy_enjooy_meta_font_svg'];
								$fnt_arr['weight'] 	= $fnt['ozy_enjooy_meta_font_weight'];//$font_weight;
								$fnt_arr['style'] 	= $fnt['ozy_enjooy_meta_font_style'];//$font_style;
		...
		*/		
		...
		{
			$background_slider_image_arr = vp_metabox('ozy_enjooy_meta_page.ozy_enjooy_meta_page_background_group.0.ozy_enjooy_meta_page_background_slider_group');
	...
	jQuery.supersized({
		...
		slides : [';
			...
			foreach($background_slider_image_arr as $img) {
				$script .= $virgul . "{image:'" . $img['ozy_enjooy_meta_page_background_slider_image'] . "', title:'', url:''}";
				$virgul = ",";
			}
$script .= '],
		progress_bar : 0,
