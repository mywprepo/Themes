...
function ozy_enqueue_stylesheets() {
	...
	wp_enqueue_script('enjooy-global-plugins', OZY_BASE_URL . 'scripts/enjooy-global-plugins.js', array('jquery'), null, true );	
	...
	if($ozy_data->menu_type === 'classic') {
		...
		wp_enqueue_style('superfish', OZY_BASE_URL . 'css/superfish.css');
	...
	/*main script file*/
	wp_enqueue_script('enjooy', OZY_BASE_URL . 'scripts/enjooy.js', array('jquery'), null, true );

	/*Following variable will be used in contrast.js*/
	wp_localize_script( 'enjooy', 'headerType', array('menu_type' => $ozy_data->menu_type, 'menu_align' => $ozy_data->menu_align) );
	...
	/*royal slider*/
	wp_register_style('royalslider', OZY_BASE_URL . 'scripts/royalslider/royalslider.css');
	wp_register_style('rs-default', OZY_BASE_URL . 'scripts/royalslider/skins/default/rs-default.css');
	wp_register_style('rs-minimal-white', OZY_BASE_URL . 'scripts/royalslider/skins/minimal-white/rs-minimal-white.css');
	wp_register_style('rs-center-white', OZY_BASE_URL . 'scripts/royalslider/skins/minimal-white/rs-center-white.css');
	wp_register_script('royalslider', OZY_BASE_URL . 'scripts/royalslider/jquery.royalslider.min.js', array('jquery'), null, true );
	
	/*fancy box*/
	wp_deregister_style('fancybox');
	wp_enqueue_style('fancybox', OZY_BASE_URL . 'scripts/megafolio/fancybox/jquery.fancybox.css');
	wp_enqueue_script('fancybox', OZY_BASE_URL . 'scripts/megafolio/fancybox/jquery.fancybox.pack.js', array('jquery'), null, true );
	if(ozy_get_option('fancbox_media') == '1') {
		wp_enqueue_script('fancybox-media', OZY_BASE_URL . 'scripts/megafolio/fancybox/helpers/jquery.fancybox-media.js', array('jquery'), null, true );
	}
	if(ozy_get_option('fancbox_thumbnail') == '1') {
		wp_enqueue_style('jquery.fancybox-thumbs', OZY_BASE_URL . 'scripts/megafolio/fancybox/helpers/jquery.fancybox-thumbs.css');
		wp_enqueue_script('fancybox-thumbs', OZY_BASE_URL . 'scripts/megafolio/fancybox/helpers/jquery.fancybox-thumbs.js', array('jquery'), null, true );
	}	
	
	/*accordion slider*/
	wp_register_style('accordion-slider', OZY_BASE_URL . 'css/accordion-slider.css');
	wp_register_script('accordion-slider', OZY_BASE_URL . 'scripts/accordion-slider/accordion-slider.min.js', array('jquery'), null, true );
	...
	/*page-portfolio.php, page-horizontal-gallery.php, page-video-gallery.php*/
	wp_register_script('horizontal-gridfolio', OZY_BASE_URL . 'scripts/horizontal-gridfolio/java/FWDGrid.js', array('jquery'), null, true );
	wp_register_style('horizontal-gridfolio', OZY_BASE_URL . 'css/horizontal-gridfolio.css');
	if (is_page_template('page-portfolio.php') || is_page_template('page-horizontal-gallery.php') || is_page_template('page-video-gallery.php')) {
		wp_enqueue_script('horizontal-gridfolio');
		wp_enqueue_style('horizontal-gridfolio');
	}
	
	/*page-accordion-gallery.php*/
	if(is_page_template('page-accordion-gallery.php')) {
	...
	if(is_page_template('page-classic-gallery.php')) {
		...
		wp_enqueue_script('classic-slider-init', OZY_BASE_URL . 'scripts/royal-full-slider-init.js', array('jquery'), null, true );		
	}
	
	/*page-megafolio-gallery.php*/
	if(is_page_template('page-megafolio-gallery.php')) {
		wp_enqueue_style('megafolio-settings', OZY_BASE_URL . 'css/megafolio.css');
		wp_enqueue_script('megafolio-init', OZY_BASE_URL . 'scripts/megafolio/js/megafolio-init.js', array('jquery'), null, true );
	}

	/*page-megafolio.php*/
	if(is_page_template('page-megafolio.php')) {
		wp_enqueue_style('megafolio-settings', OZY_BASE_URL . 'css/megafolio.css');
		wp_enqueue_script('megafolio-init', OZY_BASE_URL . 'scripts/megafolio/js/megafolio-init.js', array('jquery'), null, true );
	}
	...
	if(is_page_template('page-nearby-gallery.php')) {		
		wp_enqueue_style('royalslider', OZY_BASE_URL . 'scripts/royalslider/royalslider.css');
		wp_enqueue_style('rs-minimal-white', OZY_BASE_URL . 'scripts/royalslider/skins/minimal-white/rs-minimal-white.css');
		wp_enqueue_script('royalslider', OZY_BASE_URL . 'scripts/royalslider/jquery.royalslider.min.js', array('jquery'), null, true );
		wp_enqueue_script('classic-slider-init', OZY_BASE_URL . 'scripts/royal-full-slider-init.js', array('jquery'), null, true );	
	}

	/*page-portfolio-megafolio.php, page-gallery-megafolio.php*/
	if(is_page_template('page-portfolio-megafolio.php') || is_page_template('page-gallery-megafolio.php')) {
		wp_enqueue_style('megafolio-settings', OZY_BASE_URL . 'css/megafolio.css');
		wp_enqueue_script('megafolio-init', OZY_BASE_URL . 'scripts/megafolio/js/megafolio-init.js', array('jquery'), null, true );
	}

	/*page-gallery-megafolio.php*/
	if(is_page_template('page-gallery-megafolio.php')) {
		wp_enqueue_script('addthis-script', '//s7.addthis.com/js/300/addthis_widget.js#pubid=xa-52ebcfa8435e5887', array('jquery') );
	}
	...
	if (isset($post->post_type) && $post->post_type === 'ozy_portfolio') {
		...
		$post_format = vp_metabox('ozy_enjooy_meta_portfolio.ozy_enjooy_meta_portfolio_post_format');
		...
		/* Accordion Slider */
		else if('full-page-accordion-slider' === $post_format) {
			wp_enqueue_style('accordion-slider');
			wp_enqueue_script('accordion-slider');
		}
	...
	if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_slider') == '1') {
	...
	if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_video_self') == '1') {
		wp_enqueue_script('video-background');
	}
	
	/* YouTube Video BG */
	if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_video_youtube') == '1') {
		wp_enqueue_script('tubular-youtube', OZY_BASE_URL . '/scripts/jquery/jquery.tubular.1.0.js', array('jquery') );
	}
	
	/* Vimeo Video BG */
	if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_video_vimeo') == '1') {
...
function ozy_page_model_css($classes) {
	...
	if($use_custom_sidebar == '1') {
		$sidebar_position 	= ozy_get_metabox('sidebar_group.0.ozy_enjooy_meta_page_sidebar_position', 0, $_post_id);
		$sidebar_name 		= ozy_get_metabox('sidebar_group.0.ozy_enjooy_meta_page_sidebar', 0, $_post_id);
	}
	...
	if(is_page_template('page-accordion-menu.php') || is_page_template('page-masterslider-full.php') || is_page_template('page-countdown.php') || is_404()) {
		$ozy_data->hide_everything_but_content = true;
	}else{
		$_classes.= ' ozy-' . ozy_get_option('primary_menu_type', 'side-menu');		
	}
...
function ozy_init_test() {
	...
	$ozy_data->menu_type = ozy_get_option('primary_menu_type', 'side-menu');	
...
function ozy_init_metaboxes()
{
	...
	$ozy_enjooy_meta_portfolio_tmp 				= OZY_BASE_DIR . 'admin/metabox/portfolio.php';
	$ozy_enjooy_meta_page_tmp 					= OZY_BASE_DIR . 'admin/metabox/page.php';
	$ozy_enjooy_meta_blog_tmp 					= OZY_BASE_DIR . 'admin/metabox/blog.php';
	$ozy_enjooy_meta_blog_megafolio_tmp 		= OZY_BASE_DIR . 'admin/metabox/blog_megafolio_options.php';
	
	$ozy_enjooy_meta_font_tmp					= OZY_BASE_DIR . 'admin/metabox/ozy_custom_font.php';
	
	// Initialize the Metabox's object
	$ozy_enjooy_meta_portfolio 					= new VP_Metabox($ozy_enjooy_meta_portfolio_tmp);
	$ozy_enjooy_meta_page_tmp 					= new VP_Metabox($ozy_enjooy_meta_page_tmp);	
	$ozy_enjooy_meta_blog_tmp 					= new VP_Metabox($ozy_enjooy_meta_blog_tmp);

	$ozy_enjooy_meta_font_tmp 					= new VP_Metabox($ozy_enjooy_meta_font_tmp);
	
	// check if portfolio and video gallery whether enabled
	if(defined('OZY_ENJOOY_ESSENTIALS_ACTIVATED')) {
		$ozy_enjooy_meta_page_portfolio_options_tmp = OZY_BASE_DIR . 'admin/metabox/page_portfolio_options.php';
		$ozy_enjooy_meta_page_portfolio_options_tmp = new VP_Metabox($ozy_enjooy_meta_page_portfolio_options_tmp);

		$ozy_enjooy_meta_page_image_gallery_options_tmp = OZY_BASE_DIR . 'admin/metabox/image_gallery_megafolio_options.php';
		$ozy_enjooy_meta_page_image_gallery_options_tmp = new VP_Metabox($ozy_enjooy_meta_page_image_gallery_options_tmp);
	}
	$ozy_enjooy_meta_blog_megafolio_tmp = new VP_Metabox($ozy_enjooy_meta_blog_megafolio_tmp);
...
function ozy_get_option($opt_name, $default = null) {
	if($default) {
		if(!vp_option('vpt_ozy_enjooy_option.ozy_enjooy_' . $opt_name)) {
			return $default;
		}else{
			return vp_option('vpt_ozy_enjooy_option.ozy_enjooy_' . $opt_name);
		}
	}else{
		return vp_option('vpt_ozy_enjooy_option.ozy_enjooy_' . $opt_name);
	}
...
function ozy_get_metabox($opt_name, $default = null, $post_id = null) {
	return vp_metabox('ozy_enjooy_meta_page.ozy_enjooy_meta_page_' . $opt_name, $default, $post_id);
}
...
function ozy_enjooy_documentation() {
	//echo "<h2>" . __( 'Documentation', 'vp_textdomain' ) . "</h2>";
	//echo 'Documentation goes here';
	//wp_enqueue_script('ozy-help-iframe', OZY_BASE_URL . 'scripts/admin/help-iframe.js', array('jquery'), null, true );	
	echo '<iframe src="http://enjooy.freevision.me/help/" id="ozy-help-iframe" width="100%" height="800px" frameborder="0"></iframe>';
}
...
function ozy_ajax_load_more() {
	...
	if('folio' === $layout_type) {
		include(OZY_BASE_DIR . 'include/loop-ajax-blog-folio.php');
	}else if('metro' === $layout_type) {
		include(OZY_BASE_DIR . 'include/loop-ajax-blog-metro.php');
	}else if('megafolio' === $layout_type) {
		include(OZY_BASE_DIR . 'include/loop-ajax-blog-megafolio.php');
	}else if('image_gallery' === $layout_type) {
		include(OZY_BASE_DIR . 'include/loop-ajax-gallery-megafolio.php');
	}
...
//add_action( 'wp', 'ozy_grab_ids_from_gallery' );
	...
	function ozy_mega_menu_style_builder($json, $element_id) {
		if($json) {
			...
			if($json_obj) {
				...
				//$ozyHelper->set_footer_style('.menu-item-'. $element_id .'>.sub-container.mega{'. $style .'}');
				//#nav-primary>nav>div>ul .sub-container.mega
				//print_r($json_obj);
			}			
		}
	}
...
class BootstrapNavMenuWalker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) 
	{
		...
		$ic = 0; $type_icon_class = "";//print_r($classes);
		foreach ($classes as $c){
			
			/*if (strpos($c, 'oic-') > -1 && $depth == 0){
				unset($classes[$ic]); 
...
function ozy_check_header_slider() {
		...
		!is_page_template('page-revo-full.php') )
	{
		...
		/*Showbiz Slider*/
		$showbiz_slider_alias = ozy_get_metabox('showbiz_slider');
		if( $showbiz_slider_alias != '-1' && $showbiz_slider_alias != '' && function_exists('putShowBiz') ) {
			$slider_type = 'showbiz';
			$slider_alias 	= $showbiz_slider_alias;
		}			
	}
...
function ozy_check_footer_slider() {
	...
	{
		/*Revolution slider*/
		$revo_slider_alias = ozy_get_metabox('use_footer_slider_group.0.ozy_enjooy_meta_page_revolution_footer_slider');
		if( $revo_slider_alias != '-1' && $revo_slider_alias != '' && function_exists('putRevSlider') ) {
			$slider_type 	= 'revo';
			$slider_alias 	= $revo_slider_alias;
		}
		...
		/*Master slider*/
		$master_slider_alias = ozy_get_metabox('use_footer_slider_group.0.ozy_enjooy_meta_page_master_footer_slider');
		if( $master_slider_alias != '-1' && $master_slider_alias != '' && function_exists('masterslider') ) {
			$slider_type 	= 'master';
			$slider_alias 	= $master_slider_alias;
		}		
		...
		/*Showbiz Slider*/
		$showbiz_slider_alias = ozy_get_metabox('use_footer_slider_group.0.ozy_enjooy_meta_page_showbiz_footer_slider');
		if( $showbiz_slider_alias != '-1' && $showbiz_slider_alias != '' && function_exists('putShowBiz') ) {
			$slider_type = 'showbiz';
			$slider_alias 	= $showbiz_slider_alias;
		}			
...
function ozy_put_footer_slider($args) {
	if(is_array($args) && isset($args[0]) && $args[0]) {
		...
		echo '</div><!--#footer-slider-->'; //<div id="footer-slider-spacer" class="clear"></div> 
	}
}
...
function ozy_blog_meta_params() {
	/*post per load*/
	$post_per_load 			= (int)vp_metabox('ozy_enjooy_meta_page_blog.ozy_enjooy_meta_page_blog_count');
	
	/*order & order by*/
	$order = 'ASC'; $orderby = 'date';
	$order_orderby			= vp_metabox('ozy_enjooy_meta_page_blog.ozy_enjooy_meta_page_blog_order');
	...
	/*category filter*/
	$category_filter		= vp_metabox('ozy_enjooy_meta_page_blog.ozy_enjooy_meta_page_blog_filter');
	
	/*check if category filter set for blog page*/
	$include_categories = vp_metabox('ozy_enjooy_meta_page_blog.ozy_enjooy_meta_page_blog_category');
...
function ozy_image_gallery_meta_params() {
	// order & order by
	$order = 'ASC'; $orderby = 'date';
	$order_orderby			= vp_metabox('ozy_enjooy_meta_page_image_gallery.ozy_enjooy_meta_page_image_gallery_order');
	$order_orderby			= explode('-', $order_orderby);
	if(is_array($order_orderby) && isset($order_orderby[0]) && isset($order_orderby[1])) {
		$order = $order_orderby[1]; $orderby = $order_orderby[0];
	}					
	...
	// category filter
	$category_filter = vp_metabox('ozy_enjooy_meta_page_image_gallery.ozy_enjooy_meta_page_image_gallery_filter');
	
	// check if category filter set for blog page
	$include_categories = vp_metabox('ozy_enjooy_meta_page_image_gallery.ozy_enjooy_meta_page_image_gallery_category');
	if(is_array($include_categories) && isset($include_categories[0]) && $include_categories[0] != '-1') {
		// user not choosed to show all categories
		$include_categories = join(',', $include_categories);
	}else{
	...
	$post_per_load = (int)vp_metabox('ozy_enjooy_meta_page_image_gallery.ozy_enjooy_meta_page_image_gallery_count');
	
	// get default category
	$default_category_slug = $default_category_name = ""; $default_category_id = 0;
	$temp_arr = get_term(vp_metabox('ozy_enjooy_meta_page_image_gallery.ozy_enjooy_meta_page_image_gallery_category_default'), 'image_gallery_category', 'ARRAY_A');	
...
function ozy_portfolio_meta_params() {
	// order & order by
	$order = 'ASC'; $orderby = 'date';
	$order_orderby			= vp_metabox('ozy_enjooy_meta_page_portfolio.ozy_enjooy_meta_page_portfolio_order');
	$order_orderby			= explode('-', $order_orderby);
	if(is_array($order_orderby) && isset($order_orderby[0]) && isset($order_orderby[1])) {
		$order = $order_orderby[1]; $orderby = $order_orderby[0];
	}					
	...
	// category filter
	$category_filter = vp_metabox('ozy_enjooy_meta_page_portfolio.ozy_enjooy_meta_page_portfolio_filter');
	...
	// check if category filter set for blog page
	$include_categories = vp_metabox('ozy_enjooy_meta_page_portfolio.ozy_enjooy_meta_page_portfolio_category_sort');		
	...
	$post_per_load = (int)vp_metabox('ozy_enjooy_meta_page_portfolio.ozy_enjooy_meta_page_portfolio_count');
...
function ozy_page_meta_params($opt_param = "page") {
	...
	/*background slider*/
	$background_use_slider = ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_slider');
	...
	/*custom sidebar used?*/
	if($use_custom_sidebar == '1') {
		$sidebar_position 	= ozy_get_metabox('sidebar_group.0.ozy_enjooy_meta_page_sidebar_position');
		$sidebar_name 		= ozy_get_metabox('sidebar_group.0.ozy_enjooy_meta_page_sidebar');
	}
...
function ozy_woocommerce_meta_params() {
	...
	/*custom sidebar used?*/
	if($use_custom_sidebar == '1') {
		$sidebar_position 	= ozy_get_metabox('sidebar_group.0.ozy_enjooy_meta_page_sidebar_position', 0, $post_id);
		$sidebar_name 		= ozy_get_metabox('sidebar_group.0.ozy_enjooy_meta_page_sidebar', 0, $post_id);
	}
...
function ozy_page_master_meta_params() {
	...
	if(ozy_get_metabox('use_custom_background') == '1') {
		$meta_opt_path = 'ozy_enjooy_meta_page.ozy_enjooy_meta_page_background_group.0.ozy_enjooy_meta_page_background_video';
		if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_slider') == '1') {
			$ozyHelper->fullscreen_slide_show();
		}
		if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_video_self') == '1') {
			$ozyHelper->fullscreen_video_show(
				vp_metabox($meta_opt_path . '_self_group.0.ozy_enjooy_meta_page_background_video_self_image'),
				vp_metabox($meta_opt_path . '_self_group.0.ozy_enjooy_meta_page_background_video_self_mp4'),
				vp_metabox($meta_opt_path . '_self_group.0.ozy_enjooy_meta_page_background_video_self_webm'),
				vp_metabox($meta_opt_path . '_self_group.0.ozy_enjooy_meta_page_background_video_self_ogv')
			);
		}
		if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_video_youtube') == '1') {
			$ozyHelper->fullscreen_youtube_video_show(
				vp_metabox($meta_opt_path . '_youtube_group.0.ozy_enjooy_meta_page_background_video_youtube_image'),
				vp_metabox($meta_opt_path . '_youtube_group.0.ozy_enjooy_meta_page_background_video_youtube_id')
			);
		}
		if(ozy_get_metabox('background_group.0.ozy_enjooy_meta_page_background_use_video_vimeo') == '1') {
			$ozyHelper->fullscreen_vimeo_video_show(
				vp_metabox($meta_opt_path . '_vimeo_group.0.ozy_enjooy_meta_page_background_video_vimeo_image'),
				vp_metabox($meta_opt_path . '_vimeo_group.0.ozy_enjooy_meta_page_background_video_vimeo_id')
			);
		}	
	}
	...
	// custom page options
	$hide_page_title		= ozy_get_metabox('hide_title');
	$hide_page_content 		= ozy_get_metabox('hide_content');//ozy_enjooy_meta_page.ozy_enjooy_meta_page_
	$custom_page_title		= ozy_get_metabox('use_custom_title') == '1' ? ozy_get_metabox('use_custom_title_group.0.ozy_enjooy_meta_page_custom_title') : '';
	$custom_page_sub_title	= ozy_get_metabox('use_custom_title') == '1' ? ozy_get_metabox('use_custom_title_group.0.ozy_enjooy_meta_page_custom_sub_title') : '';
	$use_custom_sidebar		= ozy_get_metabox('use_sidebar');
	
	// generic sidebar options
	$sidebar_position		= ozy_get_option('page_page_sidebar_position');
	$sidebar_name			= ozy_get_option('page_page_sidebar_id');
	...
	$hide_page_title_arr = array(
		'page-accordion-gallery', 
		'page-classic-gallery', 
		'page-horizontal-gallery', 
		'page-megafolio-gallery', 
		'page-thumbnail-gallery',
		'page-nearby-gallery',
		'page-gallery-megafolio'
	);
...
function ozy_loader_element() {
	if(ozy_get_metabox('show_loader') == '1') {
		...
		echo '<div id="loaderMask" class="heading-font" style="position:fixed;top:0;bottom:0;left:0;right:0;width:100%;height:100%;background:#fff;"><span>0%</span></div>';
	}
}
...
function ozy_add_search_to_header_menu ( $items, $args ) {
	if(ozy_get_option('primary_menu_type', 'side-menu') !== 'side-menu' && ozy_get_option('primary_menu_search') != '-1') {
		if( 'logged-in-menu' === $args -> theme_location || 'header-menu' === $args -> theme_location ) {
			$items .= '<li class="menu-item menu-item-search"><a href="#top-search"><i class="oic-simple-line-icons-143">&nbsp;</i></a></li>';
		}
	}
	return $items;
}
