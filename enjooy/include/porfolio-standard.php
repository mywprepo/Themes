					<?php
						...
						if(vp_metabox('ozy_enjooy_meta_portfolio.ozy_enjooy_meta_portfolio_hide_meta_info') == '1') {
							$div_class = array('','',0);
						}
					?>
                    ...
                    <?php
                        //Meta info boxes
						$meta_info_boxes = vp_metabox('ozy_enjooy_meta_portfolio.ozy_enjooy_meta_portfolio_meta_info');
						if(is_array($meta_info_boxes) && count($meta_info_boxes) > 0) {
	                        foreach($meta_info_boxes as $meta_info) {
								if($meta_info['ozy_enjooy_meta_portfolio_meta_info_label']) {
    	                        	echo '<p><strong>' . $meta_info['ozy_enjooy_meta_portfolio_meta_info_label'] . ':</strong> ' . $ozyHelper->convert_to_href( $meta_info['ozy_enjooy_meta_portfolio_meta_info_value'] ) . '</p>';
								}
        	                }
						}
                        // Post category
