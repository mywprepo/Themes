...
(function (window){
	...
	var FWDSimpleDisplayObject = function(type, position, overflow, display){
		...
		if(type == "div" || type == "img" || type == "canvas" || type == "a"){
			self.type = type;	
		}else{
		...
		self.setHref = function(val){
			return self.screen.setAttribute("href", val);
		};
		
		self.setClassName = function(val){
			return self.screen.setAttribute("class", val);
		};			
		...
		self.setCSSClass = function(val){ // added by ozythemes
			self.screen.className = val;
		};		
