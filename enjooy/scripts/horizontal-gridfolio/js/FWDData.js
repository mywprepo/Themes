...
(function(window){
	...
	var FWDData = function(props){
		...
		this.parseProperties = function(){
			...
			if(self.horizontalSpaceBetweenThumbnails == undefined) self.horizontalSpaceBetweenThumbnails = 0;
			self.verticalSpaceBetweenThumbnails = self.props_obj.verticalSpaceBetweenThumbnails;
			if(self.verticalSpaceBetweenThumbnails == undefined) self.verticalSpaceBetweenThumbnails = 0;
			console.log(self.props_obj.verticalSpaceBetweenThumbnails)
			...
			for(var i=0; i<totalPlayLists; i++){
				...
				for(var j=0; j<totalChildren; j++){
					...
					//secondObj.linktogo = FWDUtils.trim(FWDUtils.getAttributeValue(mediaKid, "data-linktogo"));
					...
					obj.linkToGo = secondObj.linktogo; //ozy
				}
