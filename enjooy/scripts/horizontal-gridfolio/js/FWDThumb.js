...
(function(window){
	...
	var FWDThumb = function(
		...
		this.setupScreen = function(){
			...
			self.over_sdo = new FWDDisplayObject("div");//FWDSimpleDisplayObject("div");
			/*****************************/
			self.go_button = new FWDSimpleDisplayObject("a");
			self.go_button.setWidth(24);
			self.go_button.setHeight(24);
			self.go_button.setInnerHTML('<span class="hg-link-to-go-icon"></span>');//fa fa-share 
			self.go_button.setHref(this.linkToGo_str);
			self.go_button.setClassName('hg-link-to-go');
			self.go_button.setAlpha(1);
			if(this.linkToGo_str) self.over_sdo.addChild(self.go_button);
			/*****************************/
		...
		this.setupDescription = function(){
			...
			self.descriptionText_do.setInnerHTML(self.thumbText_str); // + '<a clas="hg-shareButton">xx</a>'
		...
		this.onMouseClickHandler = function(e){
			if(e.srcElement) e.target = e.srcElement; //IE fix
			/*console.log(e);
			e.preventDefault();*/
			//alert(e.toElement.getAttribute('class'));
			//if(typeof(e.toElement) !== 'undefined' && (e.toElement.getAttribute('class') !== null) && e.toElement.getAttribute('class').indexOf('fa-share')>-1) return;
			if(e.target.getAttribute('class') !== null && e.target.getAttribute('class').indexOf('hg-link-to-go') > -1) return;
